...
def update_domain_ns_db():
    ...
    for domain in domain_ns_result:
        ...
        verify_strategy = v[5]
        insert_time = v[6]
        ns_result.append((domain,ns_md5,domain_ns,tld_ns,ns_ns,invalid_ns,unknown_ns,verify_strategy,insert_time))

    ns_sql = 'INSERT INTO domain_valid_ns_periodic (domain,ns_md5,domain_ns,tld_ns,ns_ns,invalid_ns,unknown_ns,verify_strategy,insert_time) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) \
                ...
                unknown_ns=VALUES (unknown_ns), verify_strategy=VALUES (verify_strategy),insert_time = VALUES (insert_time)'  # 存在则更新，不存在则插入
...
def obtaining_domain_ns():
    ...
    for _ in range(3):  # 重试三次
        ...
        flag = TaskConfirm(file_name, domain_ns, 'sec', 'null').task_confirm()  # 发送探测完成消息
